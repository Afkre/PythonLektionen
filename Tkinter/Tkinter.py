...
def buttonPressed(event):
    ...
    eingabe = inp.get()                 #ermittelt die Eingabe aus dem Textfeld
    print(eingabe)
    lbl.config(text = eingabe, background="pink")  #ändert den Text von Label
    inp.delete(0, len (inp.get()))  #Löscht einen Teil des Eingabefeldes (Startindex, Endindex)

#Tkinter-Objekt wird erzeugt
window = tk.Tk()
window.title("My 1st Window")           #Titelleiste
